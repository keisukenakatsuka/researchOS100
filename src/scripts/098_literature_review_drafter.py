#!/usr/bin/env python
"""098 Literature Review Drafter — Block 6 section draft.

Generates draft_literature_review.md following the outline from 094.
...
Usage::
    ...
    python -m src.scripts.098_literature_review_drafter --run-id <id>
    python -m src.scripts.098_literature_review_drafter --run-id <id> --dry-run
"""
...
logger = logging.getLogger("098_literature_review_drafter")

_DATA_DIR = _PROJECT_ROOT / "data" / "lit_review"
_SCRIPT_ID = "098_literature_review_drafter"


def parse_args():
    p = argparse.ArgumentParser(
        prog="python -m src.scripts.098_literature_review_drafter",
        description="098 Literature Review Drafter — generates draft_literature_review.md",
    )
...
def main():
    ...
    logger.info("=== 098 Literature Review Drafter === run_id=%s", args.run_id)
    ...
    if args.dry_run:
        ...
        if lr_path.exists():
            ...
            streams = len(lr.get("theoretical_streams", []))
            findings = lr.get("empirical_findings", {})
            est = len(findings.get("established", []))
            emg = len(findings.get("emerging", []))
            con = len(findings.get("contested", []))
            oq = len(lr.get("open_questions", []))
            papers = len(lr.get("papers", []))
            print(f"\n  Streams: {streams}, Findings: {est}E/{emg}M/{con}C, Open Qs: {oq}, Papers: {papers}")

        ls_path = run_dir / "landscape.json"
        if ls_path.exists():
            ls = json.loads(ls_path.read_text())
            blindspots = len(ls.get("blindspots", []))
            hotspots = len(ls.get("hotspots", []))
            print(f"  Landscape: {blindspots} blindspots, {hotspots} hotspots")

        outline_path = run_dir / "paper_outline.json"
        if outline_path.exists():
            outline = json.loads(outline_path.read_text())
            for section in outline.get("sections", []):
                if section.get("section_id") == "literature_review":
                    print(f"  Outline target: {section.get('target_words', 'N/A')} words")

        if missing:
            print(f"\n  Missing required inputs: {missing}")
        else:
            print(f"\n  Would generate: draft_literature_review.md")
        sys.exit(0)
    ...
    if result.status == DraftStatus.GENERATED:
        update_step(
            run_dir, _SCRIPT_ID,
            status="completed",
            outputs=["draft_literature_review.md"],
        )
    else:
        update_step(
            run_dir, _SCRIPT_ID,
    ...
    # Summary
    print(f"\n{'=' * 60}")
    print(f"098 Literature Review Drafter — {result.status.value.upper()}")
    print(f"{'=' * 60}")
    ...
    if result.status == DraftStatus.GENERATED:
        ...
        print(f"\nOutput: {run_dir / 'draft_literature_review.md'}")
        sys.exit(0)
    else:
